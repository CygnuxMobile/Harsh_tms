...
import 'package:harsh/model/quick_docket_model/billing_response.dart';
import 'package:harsh/model/quick_docket_model/custList_response.dart';
import 'package:harsh/model/quick_docket_model/eway_bill_response.dart';
import 'package:harsh/model/quick_docket_model/picode.dart';
import 'package:harsh/model/quick_docket_model/product.dart';
import 'package:harsh/model/quick_docket_model/transport_response.dart';
import 'package:harsh/moduls/docket_page/docket_controller.dart';
import 'package:harsh/moduls/quick_docket_page/quick_docket_screen.dart';
import 'package:harsh/utils/logging.dart';
...
import '../../model/quick_docket_model/destination_model.dart';
...
class QuickDocketController extends GetxController {
  ...
  RxList<DestinationResponse> destinationList = <DestinationResponse>[].obs;
  ...
  eWayBillApi(
      ...
      required bool isQrScan,
      required isMenuScreen,
        ...
        if (eWayBillResponse.data.status == 1) {
          if (docketInvoiceList.length <= 1) {
            if (eWayBillStatus.value == eWayBill.withEWayBill) {
              ...
              docketDateController.text =
                  convertDateFormat(date: eWayBillResponse.data.eWayBillInvoiceDate);
  ...
  Future<void> transportModelApi({
    ...
    try {
      final dio.Response response = await WebService.tmsGetRequest(
          "${ApiService.GetTransportMode}?Custcode=$Custcode&Paybas=$Paybas");
      AppLoader().hide();
      if (response.statusCode == 200) {
        TransportResponseModel transportModelResponse =
            transportResponseModelFromJson(response.data);
  ...
  Future<void> destinationApi({required String city}) async {
    AppLoader().show();
    try {
      final dio.Response response = await WebService.tmsPostRequest(
          url: ApiService.destination,
          body: jsonEncode({
            "Filters": [
              {"FieldName": "city", "FieldValue": "${city.split("-")[0]}"}
            ]
          }));
      AppLoader().hide();
      if (response.statusCode == 200) {
        if (response.data != []) {
          destinationList.value = destinationResponseFromJson(response.data);
        }
      } else {
        if (kDebugMode) {
          print(response.statusMessage);
  ...
  Future<void> getFromToCityType({required String locationID}) async {
    try {
      final dio.Response response =
          await WebService.tmsGetRequest("${ApiService.GetFromToCity}?type=$locationID");
      if (response.statusCode == 200) {
  ...
  consignorSelectName(String value) {
    ...
    consignorId =
        customerList.where((innerValue) => innerValue.custnm.contains(value)).first.custcd;
  }
          ...
          submitAlertDialog(
            ...
            printerTap: () {
              ...
              Get.toNamed(AppRoutes.docketDetails,
                  arguments: quickDocketSubmitResponse.data.docketno);
            },
            onTapText: 'Done',
  ...
  submitValidator({
    ...
    if ((eWayBillStatus.value == eWayBill.withoutEWayBill)
